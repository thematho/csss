    ...
    */
    goto: function(which) {
      ...
      if (slide) { // Slide actually changed, perform any other tasks needed
        ...
        $$('iframe.csss-import').forEach(function(iframe) {
    ...
    adjustFontSize: function() {
      ...
      if (body.classList.contains('show-thumbnails') || slide.classList.contains('dont-resize')) {
        return;
      }

      for (
        var percent = 100;
        (scrollRoot.scrollHeight > innerHeight || scrollRoot.scrollWidth > innerWidth) && percent >= 35; percent -= 5
      ) {
      ...
      if (slide.clientHeight && slide.clientWidth) {
        ...
        for (;
          (slide.scrollHeight > slide.clientHeight || slide.scrollWidth > slide.clientWidth) && percent >= 35; percent--
        ) {
          htmlStyle.fontSize = percent + '%';
        }
